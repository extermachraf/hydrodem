...
import { Wrench, Truck, MousePointerClick } from "lucide-react";
          ...
          {services.map((service, index) => {
            ...
            const cardContent = (
              <motion.div
                ...
                onClick={service.isDiagnostic ? () => setIsDiagnosticOpen(true) : undefined}
                className={`group relative h-[500px] w-full rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer`}
              >
                {/* Background Image */}
                <Image
                  src={service.image}
            ...
            // Wrap "Service sur Terrain" in a Link, Diagnostic just uses onClick
            if (!service.isDiagnostic && service.href) {
              return (
